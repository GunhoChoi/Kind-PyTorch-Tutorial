...
# if gpu memory is not enough, change image size to a smaller one
content_layer_num = 4
image_size = 512
epoch = 10000
...
style_weight = [1/n**2 for n in [64,256,512,1024,2048]]
...
while iteration[0] < epoch:
	...
	def closure():
		...
		style_loss = [GramMSELoss().cuda()(out[i],style_target[i])*style_weight[i] for i in range(5)]
		content_loss = nn.MSELoss().cuda()(out[content_layer_num],content_target)
		total_loss = 10000 * sum(style_loss) + sum(content_loss)
		total_loss.backward()
